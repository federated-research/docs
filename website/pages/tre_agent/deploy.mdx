...
Within the DARE-TREFX-Environment1 directory the TRE Layer docker-compose and configuration files are in the `TRE` [directory](https://github.com/SwanseaUniversityMedical/DARE-TREFX-Environment1/tree/main/TRE):
...
<Callout type="info">
  ...
  [here](/five_safes_tes/TES/install_funnel)
</Callout>
### Run docker-compose
...
<Steps>
  ### Enter TRE credentials The Egress UI will prompt you to provide the tre
  credentials in order to authenticate against the `TRE API`. By default these
  are: ```yaml copy Username: accessfromegresstotre Password: password123 ```
  <Callout>
