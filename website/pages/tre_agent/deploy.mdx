...
<Callout>
  If you are doing reverse proxy for S3 (MinIO or RustFS), the `Websockets Support` should be
...
- `ServiceStack/compose-manifests/storage/s3-tre.yml` which defines the TRE Layer's S3 server.
  <Callout type="info">
    If the option `Login with KeyCloak` couldn't be shown in the login protal of
    RustFS, check health of the KeyCloak server and try restarting the RustFS container by the command `docker compose
    restart rustfs_TRE`.
  </Callout>
