...
In production, you should have been given credentials of a user with the `dare-tre-admin` role and group in the `Dare-Control` Keycloak realm from the Submission Layer admin.

Put these credentials in the page `Update Submission Credentials` in the TRE Agent UI.
...
After admin of Submission layer has created a project, added users to the project and added your TRE to the project, you need to approve the project and memberships from the TRE Agent UI (port `8989` of the host machine), at the page `Projects`.
