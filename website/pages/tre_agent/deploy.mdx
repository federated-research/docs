...
In production, the credentials of a user (who doesn't necessarily need to be a real researcher user) with the `dare-tre-admin` role and group, in the `Dare-Control` Keycloak realm, from the Submission Layer side, is needed to connect the TRE Agent to the Submission Layer.
You can create this user if you own Submission Layer. Otherwise, these credentials are typically provided by the Submission Layer admin.
