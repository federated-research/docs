...
When a user submits a TES message via the Submission layer, the request is sent to create ephemeral credentials using the Camunda workflow. The corresponding process handler then create the credentials, stores them in [Vault](/five_safes_tes/reference_implementation/core_components#vault), and signal completion back to the TRE layer. The TRE layer then pulls credentials from the Vault, injects them into TES executors as environment variables. These are then revoked using another Camunda workflow either using the set timer or when a TES task is finished or failed.

The Camunda workflows are defined using process models (or **BPMN**), and decision notations (or **DMN**).
...
2. On application startup, Zeebe’s client automatically deploys the models. It can also be deployed using the Camunda Modeler interface using the rocket button shown in the bottom left corner.
...
Currently, Five Safes TES uses three BPMN models: - one for starting credential creation, one subprocess for determining which credentials to create, one for revocation.
...
- **ServiceTask: Store Parent Process Instance Key**: Persists the root workflow instance key into the credentials DB. This is essential to persist the key because it helps finding the exact parent process instance key for the submission ID in the TRE layer, fetching the correct credentials, and avoiding duplicate entries into ephemeral credentials DB.
...
- **ServiceTask: postgres_credentials**: Invokes the Postgres credential creation handler (create-postgres-user). Additional handlers (e.g., Trino) follow the same pattern when enabled. It also created a pre-defined path in Vault where the credential information is stored safely and securely (explained in detail later).
...
This handler serves as a flexible extension point for **custom credential types** that fall outside the standard built‑ins (Postgres, Trino). Whenever the DMN decision table determines that a submission requires one of these custom credential sets, the workflow routes into this handler through the subprocess.
...
### Revoke Credentials BPMN 

This BPMN model defines how the to revoke/expire ephemeral credentials once they are no longer valid or when an explicit revoke request comes in.
...
8. TRE then triggers **Revoke** workflow (the revoke timer inside the Camunda model will revoke after TTL). If the submission is finshed or failed/cancelled due to any issues, The REVOKE workflow is triggered instantly before having to wait for the TTL timer.
