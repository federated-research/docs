...
There are three layers : `Submission` `Egress` and `TRE`, as described in [Stack Components](/setup/stackinfo).
...
For the stack to execute the analysis you need to set up a GA4GH TES implementation by following [these steps](funnel_allinone#run-analysis-with-all-in-one-demo).
 </Callout>
