...
- Deployed Five Safes TES [All-In-One Quickstart](/five_safes_tes/quickstart) or one [Submission Layer](/submission) and at least one [TRE Agent](/tre_agent) with [Funnel](/five_safes_tes/TES/install_funnel) connected to the agent.
- [OMOP CDM database](/standards#observational-medical-outcomes-partnership-omop) connected to each TRE. You can use [`omop-lite`](https://github.com/Health-Informatics-UoN/omop-lite) for a quickstart synthetic OMOP CDM.
  This OMOP CDM should be under the schema with the name is the same as the project name in the Submission Layer. For example, if the project name is "testing", the OMOP CDM should be under the schema "testing".
...
- You don't need to put the values into `postgresPassword` tag.
...
<Callout>
  Please make sure that all of the project and schema names should be in
  lowercase, due to a known issue. We are working on fixing this.
</Callout>
...
This assumes that the OMOP CDM is under the public schema. If it is under a different schema, you need to change the `public.measurement` to the actual schema name. For example, if the schema name and the project name are both `testing`, the query should be:

```sql
SELECT value_as_number FROM testing.measurement
WHERE measurement_concept_id = 21490742
AND value_as_number IS NOT NULL
```
