...
- Deployed Five Safes TES [DemoStack Quickstart](five_safes_tes/reference_implementation/quickstart) or one [Submission Layer](/submission) and at least one [TRE Agent](/tre_agent) with [Funnel](/five_safes_tes/reference_implementation/TES/install_funnel) connected to the agent.
...
<Callout type="info">
  While using `omop-lite` to create a synthetic OMOP CDM database, `OMOP CDM`'s
  schema name should be in lowercase as common naming convention.
</Callout>
...
- If you are using the `DemoStack Quickstart`, the `.env` file is located in the `DemoStack` directory.
...
<Callout>
  If you are a researcher trying to submit a analysis request to the TREs, apart
  from using steps below, you can also use the
  [Wizard](/submission/guides/teswizard) in the Submission Layer UI or a
  [Workbench](https://github.com/federated-research/5S-TES-Workbench).
</Callout>
...
# Authentication
5STES_TOKEN=<AccessToken>
5STES_PROJECT=<ProjectName>

# TRE Configuration
5STES_TRES=<ListOfTargetTREs>
  ...
  <tbody>
    ...
    <Tr>
      ...
      <Td>
        ...
        TRE. Follow this [guide](/submission/guides/submissionManagerSetupNewProject) to create a
    ...
    <Tr>
      <Td>`SQL_SCHEMA`</Td>
      <Td>
        The database schema name where the OMOP CDM is stored and the analysis will 
        be running with the query (named as `user_query`) which is located in the 
        end of the `analysis_engine.py`.

        Default is `public` and  if the OMOP CDM under a different schema, you
        need to change the `public` to the actual schema name. For example,
        if the schema name of OMOP CDM is `testing`, the .env variable should be:
        ```bash
        SQL_SCHEMA=testing
        ```
      </Td>
