...
<Callout type="info">
  This guide is expected to take approximately 30 minutes to complete with the
  assumptions that you have already set up the Five Safes TES Weave environment
  and configured the connections between the Submission Layer and TREs.
</Callout>
...
This assumes that the OMOP CDM is under the public schema. If it is under a different schema, you need to change the `public.measurement` to the actual schema name.
For example, if the schema name of OMOP CDM is `testing`, the query should be:
