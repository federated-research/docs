...
<Callout type="info"> 
- Should you want to host this OMOP CDM on the cloud Managed Instance, for example, Azure SQL DB Managed Instance or AWS RDS, please make sure to use the username (in `TRE_DATA_USER` below) and password (in `TRE_DATA_PASSWORD` below) of an user who has the `admin` role.
- In case you could not be able to obtain user with `admin` role, you can host your OMOP CDM in the same `postgresql` database instance in TRE's Docker compose.
- OMOP CDM's schema name should be in lowercase.
</Callout>
