...
The Egress application handles data egress requests. Once a task is finished executing, the results are submitted to it for approval where a TRE administrator can check them. Once results are approved/rejected Egress then sends an update to the [TRE Agent](/software/agent) API.
