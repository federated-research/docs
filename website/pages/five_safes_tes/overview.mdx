...
<div>
    ...
    2. Then, a tool called Camunda will dynamically inject the database credentials into the environment variables of the TES message, which Funnel will use to connect to the database.

    3. Then the payload is send to the Task Execution Service API (TES API) which receives code & message and translates them into the unified, platform-independent format. All together, standardised payload is pulled by Funnel to execute the analysis on the database. 
