...
- The `RedirectUri` in your OIDC configs (in the `.env` file)
- The `allowed redirect URIs` registered in Keycloak's client settings (in the Keycloak console, under `Dare-TRE-UI` client → Settings → Valid Redirect URIs)
