...
# Five Safes TES DemoStack Quickstart
...
- [Funnel](https://github.com/ohsu-comp-bio/funnel) is chosen as the TES executor for the DemoStack quickstart and it needs to be installed beforehand. Instructions can be found [here](/five_safes_tes/reference_implementation/TES/install_funnel).
...
Within the **5S-TES-deployment** directory, change to the `DemoStack` directory where the docker compose file of this stack is located:
...
While the DemoStack components are running, Funnel needs to be configured to use the TRE MinIO instance `minioTRE`.
Follow this [guide](/five_safes_tes/reference_implementation/TES/install_funnel) to set up Funnel with MinIO.
...
To send a real submission, some settings need to be adjusted in the `.env` file inside the `DemoStack` directory as follow.
