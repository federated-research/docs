...
- [Funnel](https://github.com/ohsu-comp-bio/funnel) is chosen as the TES executor for the DemoStack quickstart. The DemoStack provides automated setup scripts that handle Funnel installation and configuration. 
  ...
  <Callout type="info">
  However, If there are any issues with the funnel automated setup or funnel docker image or if you prefer to set up Funnel manually, follow the instructions [here](/five_safes_tes/reference_implementation/TES/install_funnel).
  </Callout>
...
<Callout type="info">
  The DemoStack quickstart automatically creates **a project named Testing and a TRE named DEMO**.
  Once the Docker Compose stack has started successfully, please allow approximately 5 minutes for 
  the TRE Agent to complete the migration and initialization of the demo projects.
</Callout>
