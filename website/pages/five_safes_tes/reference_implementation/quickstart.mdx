...
- [Funnel](https://github.com/ohsu-comp-bio/funnel) is chosen as the TES executor for the DemoStack quickstart. The DemoStack provides automated setup scripts that handle Funnel installation and configuration. 
  ...
  <Callout type="info">
  However, If there are any issues with the automated setup, or if you prefer to set up Funnel manually, follow the instructions [here](/five_safes_tes/reference_implementation/TES/install_funnel).
  </Callout>
...
### Start the `starter.sh` automation scripts with `DemoStack`
...
- **Permission to execute the `starter.sh` script and run it** (Only requires if you run this for the first time):
  ```bash copy
  chmod +x starter.sh
  ```
- **Run the `starter.sh` script:**
  ```bash copy
  ./starter.sh
  ```

<Callout type="info">
  Wait until all the services are up and running and the `starter.sh` script finishes executing. 
...
The DemoStack includes a starter.sh script that automates the initial setup process. This script will:
1. Start all Docker Compose services in `DemoMode = true`.
2. Install and configure the MinIO client (mc).
3. Create MinIO access keys for Funnel.
