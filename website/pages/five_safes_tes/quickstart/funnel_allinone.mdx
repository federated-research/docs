...
Access the Keycloak console at `http://localhost:8085`, using the admin credentials:
...
Change the SSL setting of each Realm (`DARE-Control`, `DARE-TRE`, `DARE-Egress`) by these steps:

1. Navigate to the `Realm settings` under `Configure` section on the left-hand side.
2. Choose `None` from the `Require SSL` dropdown.
