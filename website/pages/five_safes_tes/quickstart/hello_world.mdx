...
- [Funnel](https://github.com/ohsu-comp-bio/funnel) installed and running with Minio access configured. Instructions can be found [here](/five_safes_tes/TES/install_funnel).
...
For example, admin user `globaladminuser` to access Submission UI, TRE-UI and Data Egress UI will be seeded, `Testing` project will be created, TRE named `DEMO` will be deployed, and users credentials used to communicate between apps will be seeded.
...
You should see the processing status of the submission. Once the submission is processed, and `Requested Egress` is on waiting, follow this [guide](/egress/guides/EgressManagerApprovingRelease) to approve the submission results.
