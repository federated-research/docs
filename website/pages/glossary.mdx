...
- **REST API (Representational State Transfer API):** An architectural style for designing networked applications. REST APIs use HTTP requests to perform standard operations such as creating, reading, updating, or deleting (CRUD) resources. An example is a service requesting data from another web service.
