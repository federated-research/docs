...
|--------------|---------------|-------------|----------------------------------------------------------|----------|
| Isolated | Summary       | Manual      | Initiate: BC RQUEST and Process: BC Link or Bunny        |          |

## Use in Federated Research

**Discovery** is built to support federated research networks where each site runs its own software locally. It polls a central **Task API** (such as [Relay](https://hutch.health/relay) or RQUEST) to retrieve incoming tasks, processes them against the site’s OMOP database, and returns anonymised, aggregate results.
